...
// Convert TOML theme to YAML format
async function convertTomlToYaml(tomlFilePath) {
  try {
    const yamlContent = await fs.readFile(tomlFilePath, 'utf8');
    const yamlObject = yaml.load(yamlContent);
    return toml.stringify(yamlObject);
  } catch (err) {
    throw new Error(`Error converting YAML to TOML: ${err.message}`);
  }
}
const cli = meow(
  `
    Usage
      ...
      create a backup of alacritty's configuration file at "$HOME/.config/alacritty/backup/alacritty.theme-switch-backup.toml".
...
`,
  {
    flags: {
      ...
      backup: {
        type: 'string',
        alias: 'b',
        default: `${home}/.config/alacritty/backup/alacritty.theme-switch-backup.yml`,
      },
      select: {
        type: 'string',
        alias: 's',
...
(async () => {
  ...
  try {
    ...
    // if (!isToml(cli.flags.config)) {
    //   throw new Error(`Configuration ${cli.flags.config} is not a TOML file.`);
    // }
  ...
  try {
    ...
    const yamlThemeContent = await convertTomlToYaml(selectedThemeFile);
    await applyTheme(yamlThemeContent, cli.flags.config);
